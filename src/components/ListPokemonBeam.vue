    ...
    <v-row no-gutters>
        <v-col cols="4" 
            ...
            <router-link :to="`/pokebeam/${item.id}`">
                <div  class="tw-border-2 tw-border-sky-900
                                tw-transition tw-duration-500 tw-ease-in-out tw-delay-50 
                                tw-bg-cyan-100 tw-shadow-xl tw-shadow-inner tw-shadow-sky-600/40
                                hover:tw--translate-y-1 hover:tw-scale-110 hover:tw-duration-300
                                    tw-border-2 tw-rounded-xl justify-center align-center">
                    <v-row no-gutters class="tw-border-b-2 tw-border-indigo-900 tw-bg-cyan-50 tw-rounded-t-lg tw-shadow-inner">
                    ...
                    <v-row no-gutters class="justify-center align-center tw-pt-2 tw-text-slate-500">
                        
                        #{{ item.id }}
                        
                    </v-row>
                    <v-row no-gutters class="justify-center align-center tw-pb-5 tw-font-medium tw-text-2xl
                                        tw-capitalize tw-text-slate-700">
                        {{ item.name}}
...
const sortPokemon = pokemon.map((each) => {
    ...
    // type: each.types.map((each) => each.type.name),
        ...
        ability: each.abilities.map((each) => each.ability.name).join(", "),
        height: each.height,
        weight: each.weight,
        id: each.id,
  };
